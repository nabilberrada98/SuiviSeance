...
foreach ($resFilieres as $fil):
    array_push($seriesNames, "{name : '" . addslashes($fil['nom_filiere']) . "', y : " . $fil['avancement'] . " , drilldown : " . $fil['id'] . "}");
endforeach;
?>
<html>
    <head>
                ...
                $(document).ready(function () {
                    ...
                    chart: {
                        type: 'column'
                            ...
                            name: 'Filières',
                                    colorByPoint: true,
                                    data: [<?php echo join(',', $seriesNames); ?> ]
                            }],
                            drilldown: {
                            series: [
    <?php
    $i = 0;
    $len = count($resFilieres);
    foreach ($resFilieres as $fil):
        //$resMatiere = json_encode(array_combine($keys, array_values($fil)));
        ?>
                                {
        id: <?= $fil['id'] ?>,
                data: [
        <?php
        $matieres = $controller->getMatieresAvancement($fil['id']);
        $resMat = [];
        foreach ($matieres as $mat):
            array_push($resMat, "['" . addslashes($mat['nom']) . "'," . $mat['avancement'] . "]");
        endforeach;
        echo join(',', $resMat);
        ?>
                                        ]
